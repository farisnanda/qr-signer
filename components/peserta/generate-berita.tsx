    ...
    <div className="space-y-4">
      {stage === "input" && (
        <form onSubmit={buatPreview} className="space-y-3">
          <div>
            <label className="mb-1.5 block text-sm font-semibold text-slate-900">PIN Sesi</label>
            <input
              value={pin}
              onChange={(e) => setPin(e.target.value)}
              inputMode="numeric"
              className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-center text-lg font-semibold tracking-[0.25em] text-slate-950 outline-none transition placeholder:text-sm placeholder:font-medium placeholder:tracking-normal placeholder:text-slate-500 focus:border-blue-600 focus:ring-4 focus:ring-blue-100"
              placeholder="PIN dari panitia"
            />
            <p className="mt-2 text-xs font-medium text-slate-700">Masukkan PIN yang diberikan panitia di lokasi.</p>
          </div>
